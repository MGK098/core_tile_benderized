...
#(
    ...
    typedef enum logic [1:0] {
        IDLE, SEND_AR, RECEIVING, DELIVER
    } state_t;
    ...
    // ----------------------------------------------------------------
    // CHANGE 2: axi_len/size  mux req_i (IDLE) vs req_q (elsewhere)
    // ----------------------------------------------------------------
    logic [7:0] axi_len;
    logic [2:0] axi_size;
    always_comb begin
        automatic hpdcache_mem_size_t cur_size =
            (state_q == IDLE && req_valid_i) ? req_i.mem_req_size
                                             : req_q.mem_req_size;
        if (cur_size >= hpdcache_mem_size_t'(AXI_BYTES_LOG2)) begin
            axi_len  = 8'((1 << (cur_size - AXI_BYTES_LOG2)) - 1);
            axi_size = 3'(AXI_BYTES_LOG2);
        end else begin
            axi_len  = 8'h00;
            axi_size = cur_size;
    ...
    always_comb begin
        ...
        case (state_q)
            ...
            DELIVER:   if (resp_ready_i)                  state_d = IDLE;
            default:   state_d = IDLE;
        endcase
    end

    hpdcache_mem_error_e err_next;
    always_comb begin
        case (axi_r_i.resp)
            axi_pkg::RESP_SLVERR,
            axi_pkg::RESP_DECERR: err_next = HPDCACHE_MEM_RESP_NOK;
            default:              err_next = err_q;
        endcase
    end
        ...
        if (!rstn_i) begin
            ...
            case (state_q)
                ...
                RECEIVING: begin
                    if (axi_r_valid_i) begin
                        if (axi_len == 0) begin
                            // Single-beat UC read: place data at correct word position
                            line_q <= '0;
                            line_q[req_q.mem_req_addr[LINE_BYTES_LOG2-1:AXI_BYTES_LOG2]
                                   * AXI_DATA_WIDTH +: AXI_DATA_WIDTH] <= axi_r_i.data;
                        end else begin
                            // Multi-beat cache line refill: shift in sequentially
                            line_q <= {axi_r_i.data, line_q[LINE_W-1 : AXI_DATA_WIDTH]};
                        end
                        err_q <= err_next;
                    end
                end
    ...
    // ----------------------------------------------------------------
    // CHANGE 3: fold lock/cache into axi_ar_o, mux req_i vs req_q
    // ----------------------------------------------------------------
    always_comb begin
        automatic hpdcache_mem_req_t req_sel =
            (state_q == IDLE && req_valid_i) ? req_i : req_q;
        automatic logic            lock_sel;
        automatic axi_pkg::cache_t cache_sel;

        lock_sel  = (req_sel.mem_req_command == HPDCACHE_MEM_ATOMIC) &&
                    (req_sel.mem_req_atomic  == HPDCACHE_MEM_ATOMIC_LDEX);
        cache_sel = req_sel.mem_req_cacheable ?
                    (axi_pkg::CACHE_BUFFERABLE | axi_pkg::CACHE_MODIFIABLE |
                     axi_pkg::CACHE_RD_ALLOC   | axi_pkg::CACHE_WR_ALLOC)  :
                    axi_pkg::CACHE_MODIFIABLE;

        axi_ar_o        = '0;
        axi_ar_o.id     = req_sel.mem_req_id;
        axi_ar_o.addr   = req_sel.mem_req_addr;
        axi_ar_o.len    = axi_len;
        axi_ar_o.size   = axi_size;
        axi_ar_o.burst  = axi_pkg::BURST_INCR;
        axi_ar_o.lock   = lock_sel;
        axi_ar_o.cache  = cache_sel;
        axi_ar_o.prot   = '0;
        axi_ar_o.qos    = '0;
        axi_ar_o.region = '0;
        axi_ar_o.user   = '0;
    ...
    // ----------------------------------------------------------------
    // CHANGE 4: assert AR combinationally from IDLE  no 1-cycle bubble
    // ----------------------------------------------------------------
    assign axi_ar_valid_o = (state_q == SEND_AR)
                          || (state_q == IDLE && req_valid_i);

    assign req_ready_o   = (state_q == IDLE);
    assign axi_r_ready_o = (state_q == RECEIVING);

    assign resp_valid_o            = (state_q == DELIVER);
    assign resp_o.mem_resp_r_data  = line_q;
    assign resp_o.mem_resp_r_id    = req_q.mem_req_id;
    assign resp_o.mem_resp_r_last  = 1'b1;
    assign resp_o.mem_resp_r_error = err_q;
...
#(
    ...
    always_comb begin
        state_d = state_q;
        case (state_q)
            IDLE:      if (req_valid_i)                   state_d = SEND_AR;
            SEND_AR:   if (axi_ar_ready_i)                state_d = RECEIVING;
            RECEIVING: if (axi_r_valid_i && axi_r_i.last) state_d = DELIVER;
    ...
    always_ff @(posedge clk_i or negedge rstn_i) begin
        if (!rstn_i) begin
            ...
            case (state_q)
                ...
                RECEIVING: begin
                    if (axi_r_valid_i) begin
                        //if (req_q.mem_req_len == 0) begin
                        if (axi_len == 0) begin
                            // Single-beat UC read: place data at correct word
                            // position based on address bits [LINE_BYTES_LOG2-1:AXI_BYTES_LOG2]
                            line_q <= '0;
                            line_q[req_q.mem_req_addr[LINE_BYTES_LOG2-1:AXI_BYTES_LOG2]
                                   * AXI_DATA_WIDTH +: AXI_DATA_WIDTH] <= axi_r_i.data;
                        end else begin
    ...
    assign lock  = (req_q.mem_req_command == HPDCACHE_MEM_ATOMIC) &&
                   (req_q.mem_req_atomic  == HPDCACHE_MEM_ATOMIC_LDEX);

    assign cache = req_q.mem_req_cacheable ?
                   (axi_pkg::CACHE_BUFFERABLE | axi_pkg::CACHE_MODIFIABLE |
                    axi_pkg::CACHE_RD_ALLOC   | axi_pkg::CACHE_WR_ALLOC)  :
                   axi_pkg::CACHE_MODIFIABLE;
    ...
    always_comb begin
        axi_ar_o         = '0;
        axi_ar_o.id      = req_q.mem_req_id;
        axi_ar_o.addr    = req_q.mem_req_addr;
        axi_ar_o.len     = axi_len;
        axi_ar_o.size    = axi_size;
        axi_ar_o.burst   = axi_pkg::BURST_INCR;
        axi_ar_o.lock    = lock;
...
*/
/*
// caused isssied with single beat uc req
module hpdcache_mem_to_axi_read
import hpdcache_pkg::*;
import axi_pkg::*;
#(
...
*/
//issue with real time fetch in cheshire with size / lenght
/*
module hpdcache_mem_to_axi_read
import hpdcache_pkg::*;
import axi_pkg::*;
#(
    ...
    always_comb begin
        ...
        axi_ar_o.addr    = req_q.mem_req_addr;
        axi_ar_o.len     = axi_len;
        axi_ar_o.size    = req_q.mem_req_size;
        axi_ar_o.burst   = axi_pkg::BURST_INCR;
        axi_ar_o.lock    = lock;
        axi_ar_o.cache   = cache;
        axi_ar_o.prot    = '0;
