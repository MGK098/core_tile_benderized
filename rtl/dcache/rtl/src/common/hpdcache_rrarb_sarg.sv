...
`endif
    ...
    //  {{{
    hpdcache_prio_1hot_encoder_sarg #(.N(N)) prio_msk_i   (.val_i(req_i & mask), .val_o(gnt_msk));
    hpdcache_prio_1hot_encoder_sarg #(.N(N)) prio_nomsk_i (.val_i(req_i)       , .val_o(gnt_nomsk));
